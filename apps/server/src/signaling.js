...
import { WS_PING_INTERVAL_MS } from '@lancam/shared';
const log = createLogger('signaling');
export class SignalingServer {
    sessionManager;
    wss;
    peers = new Map(); // peerId → connection
    wsToPeer = new Map(); // ws → peerId
    pingInterval = null;
    constructor(serverOrServers, sessionManager) {
    ...
    handleCameraStatus(ws, msg) {
        const peerId = this.wsToPeer.get(ws);
        if (!peerId)
            return;
        const peer = this.peers.get(peerId);
        if (!peer || peer.role !== 'camera')
            return;
        this.sessionManager.updateCameraStatus(peer.sessionId, peerId, msg.status);
    ...
    broadcastCameraListUpdate(sessionId) {
